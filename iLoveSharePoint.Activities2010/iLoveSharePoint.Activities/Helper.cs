...
namespace ILoveSharePoint.Workflow.Activities
{
    public static class Helper
    {
        ...
        internal static NetworkCredential GetNetworkCredentials(SPSite site, string user, string password, string secureStoreAppId)
        {
            ...
            {
                credentials.Password = Helper.DecryptString(_password.Replace(Constants.EncryptedPasswordPrefix, ""));
            }
            else
            {
        ...
        public static void ImportCryptoKey(byte[] key)
        {
            SPFarm.Local.Properties[Constants.CryptoKey] = key;
            SPFarm.Local.Update(true);
        }
        ...
        public static void SetPowerActivityScriptSingningRequired(bool signingRequired)
        {
            if (SPFarm.Local.Properties.ContainsKey(Constants.PowerActivitySigningRequired))
            {
                SPFarm.Local.Properties[Constants.PowerActivitySigningRequired] = signingRequired;
            }
            else
            {
                SPFarm.Local.Properties.Add(Constants.PowerActivitySigningRequired, signingRequired);
            }

            SPFarm.Local.Update(true);
        }

        public static bool VerifySignature(string script, string signature)
        {
            bool isValid = false;
            ...
            SPSecurity.RunWithElevatedPrivileges(delegate()
            {
                ...
                {   
                    ...
                    string dataToSign = reg.Replace(script,String.Empty);
                    ...
                    isValid = rsaAlg.VerifyData(Encoding.UTF8.GetBytes(dataToSign),sha, Convert.FromBase64String(signature));
        ...
        internal static string QueryXml(XPathNavigator xPathNav, string xPath, string selectionType)
        {
            ...
            if (obj != null)
            {
                if (obj is XPathNodeIterator)
                {
                    ...
                    {
                        result += Helper.GetValueFromXPathNav(iterator.Current, selectionType);
                ...
                {
                    result = Helper.GetValueFromXPathNav((obj as XPathNavigator), selectionType);
                }
                else
                {
                    result = obj.ToString();
        ...
        public static SPList GetExternalEventList(SPWeb web)
        {
            return web.GetListFromUrl(Constants.WaitForExternalEventListUrl);
        }
    }
}
